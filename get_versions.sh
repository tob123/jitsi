#!/bin/bash
JITSI_VERSION_LIST=`curl -s https://download.jitsi.org/stable/ | grep jitsi-meet_ | sort -V | awk -F'"' {'print $2'} | awk -F_ {'print $2'}`
...
COTURN_VERSION=`curl -s https://git.alpinelinux.org/aports/plain/community/coturn/APKBUILD?h=3.23-stable | grep ^pkgver | awk -F '=' {'print $2'}`
